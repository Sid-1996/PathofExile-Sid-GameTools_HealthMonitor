...
- **Qt 遷移進度：** Phase 1-3（deps/skeleton/StatusTab）+ Phase 4 MonitorTab 已完成並 push：`src/qt/monitor.py`（UI+觸發列表+預覽+`_SelectionOverlay` 框選+thread-safe signal 更新）、`src/qt/monitor_dialogs.py`（血條校準/介面UI閾值）、`src/qt/main_window.py`（`monitor_health` 迴圈 + `press_key_sequence` + start/stop 狀態）。smoke 含 `SMOKE MONITOR LOOP OK`。待辦：Phase 5 InventoryTab、6 ComboTab、7 Help/About/Version、8 SkillTimerModule、9 刪 tk + 打包。
