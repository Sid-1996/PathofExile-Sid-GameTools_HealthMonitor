...
## 歷史版本 - 背包排除格子功能 (2026-06-07)

### ✨ 新增

- **背包排除格子功能**: 在背包預覽中點擊格子可切換排除狀態，F3 一鍵清包時自動跳過被排除的格子
- 排除設定即時反映在預覽畫面（藍色邊框 + 對角線標記）
- 排除設定自動儲存，重啟後仍保留
- **視窗激活保護**: 所有預覽在遊戲視窗未激活時顯示「等待遊戲視窗激活」替代畫面
- **排除疊加層**: Canvas 獨立疊加層繪製排除標記，效能優化

### 🐛 修復

- 背包預覽顯示元件改為 `tk.Canvas`，解決 `ttk.Label` 無法可靠接收滑鼠點擊事件的問題
- 監控迴圈視窗檢查扁平化，無 busy-wait
- Canvas 尺寸鎖定，解決預覽區格子被截斷

### 📚 文檔

- 更新 CHANGELOG、RELEASE_NOTES、AGENTS.md

---

## 歷史版本 - 模組化重構與技能計時器 (2026-05-15)
...
- 清除含 emoji 的 `print()` 輸出，修復 Windows cp950 終端機 UnicodeEncodeError
...
- **技能計時器模組**: 新增 `skill_timer.py`，支援自訂技能槽位、毫秒級精確間隔、獨立循環
- **中斷睡眠機制**: 新增 `_interruptible_sleep()` 方法，提升停止響應速度
- **統一選擇完成 GUI 恢復**: 新增 `finalize_selection_restore_gui()` helper

### 📚 文檔

- 全面更新 AGENTS.md、PROJECT_STRUCTURE.md、DEVELOPER_HANDBOOK.md
- 清理 .windsurf/ 舊 workflow 文件

### 🏗️ 內部整理

- 模組化拆分: config_manager、language_system、utils、custom_dialogs 完全獨立
- 雙語支援全面整合
- 清理 tools/build.py dead code，版本號更新至 1.0.8

---

## 歷史版本 - 性能優化與穩定性提升版 (2026-04-29)

### ⚡ 性能優化

#### 🚀 啟動速度大幅提升
- **移除啟動時即時截圖**: 優化 `auto_load_preview()` 函數，啟動時僅載入已保存的預覽圖片，不進行耗時的螢幕截圖操作
- **載入邏輯優化**: 簡化預覽圖片載入流程，移除不必要的即時擷取邏輯，啟動時間縮短 2-5 秒

#### 🛑 停止響應即時化
- **可中斷睡眠機制**: 新增 `_interruptible_sleep()` 方法，使用 10ms 小間隔循環檢查停止狀態
- **監控線程優化**: 替換所有長時間 `time.sleep()` 調用，停止按鈕現在能在 10ms 內響應（之前可能延遲 100ms-1秒）
- **線程管理改善**: 更平滑的線程停止流程，減少系統資源占用

### 🛡️ 穩定性與用戶體驗提升

#### 🎯 UI選擇流程統一化
- **統一恢復邏輯**: 新增 `finalize_selection_restore_gui()` helper 函數，統一所有選擇完成後的GUI恢復邏輯
- **訊息顯示優化**: 確保成功訊息在GUI恢復後顯示，避免對話框被隱藏
- **代碼重構**: 減少重複代碼，提升維護性

#### 🔧 系統穩定性改進
- **錯誤處理強化**: 增強各個選擇流程的錯誤處理機制
- **資源清理優化**: 改善視窗和監聽器的清理邏輯
- **記憶體管理**: 優化圖片資源的管理，減少記憶體洩漏風險

### 📊 版本比較表更新

| 功能 | v1.0.0 | v1.0.1 | v1.0.2 | v1.0.3 | v1.0.4 | v1.0.5 | v1.0.6 | v1.0.7 | v1.0.8 |
|------|--------|--------|--------|--------|--------|--------|--------|--------|--------|
| 血魔監控 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 一鍵清包 | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 技能連段 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
