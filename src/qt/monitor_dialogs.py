...
class AdjustColorsDialog(QDialog):
    ...
    def __init__(self, app, parent=None):
        ...
        _row(color_grid, 0, g("red_h_range"), f"{app.red_h_range}", ERROR)
        _add_entry(color_grid, 1, _entry(app.red_h_range, int_validator), "red_h_range_explanation", g)
        _row(color_grid, 3, g("green_h_range"), f"{app.green_h_range}", SUCCESS)
        ...
        # HSV 微調 — 紅/綠分區避免同 grid 同 row 覆蓋（英文長文案疊影，見截圖）
        red_hsv_box = _section(g("hsv_fine_tuning") + " — " + g("red_min_saturation").rstrip(":"))
        red_hsv_grid = QGridLayout(red_hsv_box)
        red_hsv_grid.setContentsMargins(12, 12, 12, 12)
        red_hsv_grid.setColumnStretch(0, 1)
        red_hsv_grid.setColumnStretch(1, 0)
        _row(red_hsv_grid, 0, g("red_min_saturation"), f"{app.red_saturation_min}", ERROR)
        _add_entry(red_hsv_grid, 1, _entry(app.red_saturation_min, int_validator), None, g)
        _row(red_hsv_grid, 2, g("red_min_brightness"), f"{app.red_value_min}", ERROR)
        _add_entry(red_hsv_grid, 3, _entry(app.red_value_min, int_validator), "red_hsv_explanation", g)
        content_layout.addWidget(red_hsv_box)

        green_hsv_box = _section(g("hsv_fine_tuning") + " — " + g("green_min_saturation").rstrip(":"))
        green_hsv_grid = QGridLayout(green_hsv_box)
        green_hsv_grid.setContentsMargins(12, 12, 12, 12)
        green_hsv_grid.setColumnStretch(0, 1)
        green_hsv_grid.setColumnStretch(1, 0)
        _row(green_hsv_grid, 0, g("green_min_saturation"), f"{app.green_saturation_min}", SUCCESS)
        _add_entry(green_hsv_grid, 1, _entry(app.green_saturation_min, int_validator), None, g)
        _row(green_hsv_grid, 2, g("green_min_brightness"), f"{app.green_value_min}", SUCCESS)
        _add_entry(green_hsv_grid, 3, _entry(app.green_value_min, int_validator), "green_hsv_explanation", g)
        content_layout.addWidget(green_hsv_box)
