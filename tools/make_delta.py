...
由 release.ps1 在 build + 壓整包 ZIP 之後呼叫，輸出：
- <out_dir>/GameTools_HealthMonitor-delta.zip  差異壓縮檔（GitHub release asset）
...
DELTA_ASSET = "GameTools_HealthMonitor-delta.zip"
FULL_ASSET = "GameTools_HealthMonitor.zip"
...
def _updater():
    ...
    return importlib.import_module("updater_core")
...
def run(
    ...
    if prev_ok:
        ...
        if changed or added or removed:
            ...
            if full_size and delta_zip.stat().st_size > full_size * _DELTA_MAX_FRACTION:
                ...
                publish_delta = True
                print(f"[make_delta] 產出 {delta_zip.name}: {delta_zip.stat().st_size / 1024:.0f} KB (變更 {len(changed)} / 新增 {len(added)} / 刪除 {len(removed)})")
        else:
    ...
    (repo_root / "manifest.json").write_text(json.dumps(new, ensure_ascii=False, indent=1), encoding="utf-8")
    delta_info = {
        "version": version,
    ...
    (repo_root / "delta_info.json").write_text(json.dumps(delta_info, ensure_ascii=False, indent=1), encoding="utf-8")
...
def _demo():
    tmp = Path(tempfile.mkdtemp(prefix="gtool_mkdelta_demo_"))
    try:
        old = tmp / "old"
        new = tmp / "new"
        out = tmp / "out"
        ...
        updater = _updater()
        prev = updater.build_manifest(old, "1.2.1")
        (repo / "manifest.json").write_text(json.dumps(prev, ensure_ascii=False), encoding="utf-8")

        run("1.2.2", "1.2.1", new, out, repo, repo / "manifest.json")
        ...
        assert manifest["base_version"] == "1.2.1", manifest
        assert manifest["removed"] == [], manifest["removed"]
        info = json.loads((repo / "delta_info.json").read_text(encoding="utf-8"))
        assert info["version"] == "1.2.2" and info["base_version"] == "1.2.1", info
